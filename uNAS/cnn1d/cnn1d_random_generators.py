...
def random_conv1d_layer_type(block_idx, layer_idx, layer_type, relu_prob=0.9, pre_pool_prob=0.25):
    ...
    if layer_type == "Conv1D":
        ...
        layer["1x_stride"] = schema[f"conv{i}-l{j}-1x-stride"].uniform_random_value()
    elif layer_type == "1x1Conv1D":
        layer["filters"] = schema[f"conv{i}-l{j}-filters"].uniform_random_value()
    elif layer_type == "DWConv1D":
        layer["ker_size"] = schema[f"conv{i}-l{j}-ker-size"].uniform_random_value()
        layer["1x_stride"] = schema[f"conv{i}-l{j}-1x-stride"].uniform_random_value()
    else:
...
def random_conv1d_layer(block_idx, layer_idx):
    schema = get_schema()
    layer_type = schema[f"conv{block_idx}-l{layer_idx}-type"].uniform_random_value()
    return random_conv1d_layer_type(block_idx, layer_idx, layer_type)


def random_conv1d_block(block_idx, num_layers=None, branch_prob=0.2):
    ...
    for j in range(num_layers):
        layer = random_conv1d_layer(i, j)
        block["layers"].append(layer)

    return block


def random_pooling_1d():
    schema = get_schema()
    return {
...
def random_arch_1d(pooling_prob=0.9):
    """
