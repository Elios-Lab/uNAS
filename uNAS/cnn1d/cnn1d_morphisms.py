...
from .cnn1d_random_generators import random_conv1d_layer_type, random_conv1d_layer, random_pooling_1d, \
    random_dense_block, random_conv1d_block
...
def add_random_block(arch):
    ...
    arch["conv_blocks"].append(random_conv1d_block(block_idx, num_layers=1))
    return arch
...
def add_conv_layer(arch, block_idx):
    ...
    block["layers"].append(random_conv1d_layer(block_idx, layer_idx))
    return arch
...
def add_random_conv_layer(arch, block_idx):
    ...
    block["layers"].insert(layer_idx, random_conv1d_layer(block_idx, layer_idx))
    return arch
...
def change_pooling(arch):
    ...
    if arch["pooling"]:
        ...
        arch["pooling"] = random_pooling_1d()
    return arch
...
def change_conv_layer_type(arch, block_idx, layer_idx):
    ...
    for lt in layer_types:
        arch = deepcopy(arch)
        arch["conv_blocks"][i]["layers"][j] = random_conv1d_layer_type(i, j, lt)
        morphs.append(arch)
    return morphs
...
def produce_all_morphs(arch: Cnn1DArchitecture) -> List[Cnn1DArchitecture]:
    schema = get_schema()
    ...
    for i, conv_block in enumerate(parent["conv_blocks"]):
        ...
        for j, conv_layer in enumerate(conv_block["layers"]):
            ...
            if conv_layer["type"] in ["Conv1D", "1x1Conv1D"]:
            ...
            if conv_layer["type"] in ["Conv1D", "DWConv1D"]:
                ...
                morphs.append(change_conv_layer_boolean_property(parent, i, j, "1x_stride"))
    ...
    return [Cnn1DArchitecture(m) for m in morphs]
